...
class MisMensajes extends StatelessWidget {
  ...
  MisMensajes({Key key, @required this.peerId, @required this.peerAvatar}) : super(key:key);
  @override
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void onFocusChange(){
    if (focusNode.hasFocus){
      //ocultar el sticker cuando aparezca el teclado
      setState(() {
       isShowSticker= false; 
      });
    }
  ...
  void getSticker(){
    //se oculta el teclado cuando aparece el sticker
    focusNode.unfocus();
    setState(() {
  ...
  void onsendMessage(String content, int type){
    if(content.trim() != ''){
      ...
      Fluttertoast.showToast(msg: 'Nada para enviar');
    }
  }

  Widget buildItem(int index, DocumentSnapshot document){
    if (document['idFrom'] == id){
      //derecha, mis mensajes
      return Row(
        children: <Widget>[
          document['type'] == 0
          //text
            ...
            child: Material(
              child: CachedNetworkImage(
                ...
                errorWidget: (context, url, error) => Material(
                  child: Image.asset(
                    'assets/',//falta completar//imagen de error cargar en assets
                    width: 200.0,
                    height: 200.0,
                    fit: BoxFit.cover,
                  ),
                        ...
                        ? Container(
                            child: Material(
                              child: CachedNetworkImage(
                                ...
                                errorWidget: (context, url, error) => Material(
                                      child: Image.asset(
                                        'assets/',//falta colocar imagen de error
                                        width: 200.0,
                                        height: 200.0,
                                        fit: BoxFit.cover,
                                      ),
